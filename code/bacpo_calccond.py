...
#det s compatibility q1*s^2-q1*(s0-c)*s+(da/(1-q2))=0
def rad_s(a,q1,q2,c,d,s0):
    bee=float(q1*(c-s0)) #b in quadratic formula ds^2+bs+c=0
    cee=float(d/((1-q2)*a)) #c in quadratic formula
    radicand= float(bee*bee - (4*cee*q1)) #needs to be pos for real solutions
    return radicand

def solve_s(q1,q2,d,k1,s0,c,radicand):
    bee=float(q1*(c-s0))
    sol1= ((-bee)+(math.sqrt(radicand)))/(2*q1)
    sol2= ((-bee)-(math.sqrt(radicand)))/(2*q1)
    print "sol1: "
