...
%{\dot t}&=&-Dt\\
{\dot x_1}&=&x_1\left((1-q_1)f(s,e)-D-k_1t\right)\label{s4-red}
...
$x_{1}^u$ and $x_{1}^s$ in the interval $(0,(1-q_1)s)$, with 
...
In all of the following simulations $f(s,e)=a s e$ is a linear function, with $a=1.0$. Additionally, for all simulations, $s^0=1.0$, $D=0.01$, $k_1=0.015$, $k_3=0.99$, $q_1=0.24$, and $q_2=0.25$.
...
\end{subfigure}
\caption{Time series for system $(\ref{s1-red})-(\ref{s4-red})$ illustrating the two stable steady states for that system: The washout steady sate $E_0$ (right panel) and the cooperator persistence steady state $E_1^s$ (left panel).}
...
\caption{Time series for the system $(\ref{l1})-(\ref{l4})$ illustrating the steady state $E_{1,2}^s$.}
...
\end{subfigure}
\caption{Time series for system $(\ref{s1})-(\ref{s6})$ illustrating resistance to the invasion of cheaters (left) or Tragedy (right).}
\label{fig:timeser_x1}
\end{figure}
\FloatBarrier
...
\bibitem{wenseleers-bees}Wenseleers, T.,  and  Ratnieks, F.L.W., Tragedy of the commons in {\it Melipona} bees, Proc. R. Soc. Lond. B (Suppl.) 271, S310ÐS312, 2004.
